...
mod deadline {
    pub fn parse_iso8601_to_timestamp(s: &str) -> anyhow::Result<u64> {
        chrono::DateTime::parse_from_rfc3339(s).map(|dt| dt.timestamp() as u64).map_err(|e| anyhow::anyhow!(e))
    }
    pub fn format_timestamp(ts: u64) -> String {
        chrono::DateTime::from_timestamp(ts as i64, 0).map(|dt| dt.to_rfc3339()).unwrap_or_default()
    }
}
